...
#'                      each iteration.
#' @return              A list of two tibbles: One with accuracy measurements
#'                      and the other with the samples (including in which
#'                      iteration they were included in the model training).
  ...
  for (i in seq_len(n_iterations)) {
    ...
    my_samples <- my_samples %>%
      dplyr::mutate(label = dplyr::if_else(sample_id %in% new_sample_ids,
                                           oracle_label,
                                           label),
                    iteration = dplyr::if_else(sample_id %in% new_sample_ids,
                                               i,
                                               iteration))
  }
  return(list(accuracy = accuracy_tb,
              samples = my_samples))
}
...
start_sample_set <- function(samples_tb, n_samples) {
  ...
  my_samples %>%
    dplyr::mutate(label = dplyr::if_else(sample_id %in% initial_vec,
                                         oracle_label,
                                         NA_character_),
                  iteration = dplyr::if_else(sample_id %in% initial_vec,
                                             0L,
                                             NA_integer_)) %>%
    return()
}
